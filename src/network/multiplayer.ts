...
export class Multiplayer {
  ...
  private onSpawnLocal: (type: string, x: number, y: number) => void;

  constructor(
    scene: Container,
    onSpawnLocal: (type: string, x: number, y: number) => void,
  ) {
    this.scene = scene;
  ...
  private handlePlayers = (players: Record<string, PlayerState>) => {
    ...
    for (const id in players) {
      const p = players[id];
      if (id === socket.id) {
        this.onSpawnLocal(p.character, p.x, p.y);
      } else {
  ...
  private spawnRemotePlayer(
    ...
    if (existing) {
      // If they are the SAME character, just update position (optional) and return
      // We check the constructor name or add a 'type' property to your BaseCharacter
      // For simplicity, let's assume if we get a spawn request during active game,
      // we might want to ensure the sprite is correct.

      // Simple check: Is the sprite class consistent?
      const isInfectedSprite = existing instanceof Infected;
      const wantsInfected = character === "infected";

      if (isInfectedSprite === wantsInfected) {
        // Same type, no need to respawn.
        return;
      }
      // DIFFERENT TYPE? Delete old and respawn!
      this.scene.removeChild(existing);
      delete this.otherPlayers[id];
    }
